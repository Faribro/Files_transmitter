...
// Reads connection string from Vercel environment variable AZURE_STORAGE_CONNECTION_STRING
...
export async function GET(request: NextRequest) {
  ...
  if (!connectionString) {
    return NextResponse.json(
      { error: 'AZURE_STORAGE_CONNECTION_STRING not configured in Vercel environment variables' },
      { status: 503 }
    )
  }
